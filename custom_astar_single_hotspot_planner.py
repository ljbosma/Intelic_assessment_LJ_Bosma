...
class Planner:
    ...
    def bounded_astar(self, start, goal, time_limit_ms, steps_remaining):
        start_time = time.perf_counter()
        best_path = self.greedy_baseline_path(start, goal)
        best_avg = self.evaluate_path(best_path)
        ...
        while (time.perf_counter() - start_time) * 1000 < time_limit_ms:
            ...
            if step_budget > steps_remaining:
                break

            open_set = []
            start_reward = int(np.floor(self.grid.float_grid[start]))
            heapq.heappush(open_set, (0, 1, -start_reward, start, [start]))
            visited = {}
            ...
            while open_set:
                ...
                f, steps, neg_reward, pos, path = heapq.heappop(open_set)
                reward = -neg_reward
                ...
                if pos == goal:
                    avg_val = reward / steps
                    if avg_val > best_avg:
                        best_avg = avg_val
                        best_path = path
                    continue

                if pos in visited and visited[pos] >= reward:
                    continue
                visited[pos] = reward

                current_dist = self.chebyshev(pos, goal)
                x, y = pos
                for dx in [-1, 0, 1]:
                    for dy in [-1, 0, 1]:
                        if dx == 0 and dy == 0:
                        ...
                        if 0 <= nx < self.grid.N and 0 <= ny < self.grid.N:
                            ...
                            if self.chebyshev(new_pos, goal) > current_dist:
                                continue
                            new_steps = steps + 1
                            if new_steps > step_budget or new_steps > steps_remaining:
                                continue
                            new_reward = reward + int(
                                np.floor(self.grid.float_grid[new_pos])
                            )
                            h = self.chebyshev(new_pos, goal)
                            priority = new_steps + h - (new_reward / new_steps)
                            heapq.heappush(
                                open_set,
                                (
                                    priority,
            ...
            if max_extra < max_allowed_extra:
                max_extra += 1
            else:
                break  # 🔑 stop if extra steps exceed 10% of distance
    ...
    def plan_step(self, time_limit_ms: int, steps_remaining: int):
        if not self.active_path:
            ...
            if hotspot:
                self.active_path = self.bounded_astar(
                    self.drone.position, hotspot, time_limit_ms, steps_remaining
                )
                # print(f"Path found!")
...
if __name__ == "__main__":
    ...
    sim = Simulation(
        ...
        start_positions=[
            (0, 0)
        ],  # set to None if we want to randomly set position on an edge. Set to [(x, y)] if you want to specify its starting position
    ...
    print("Path lengths:", [len(p) for p in results["paths"]])
